...
export default function HyperPopArtistsPage() {
  const [favorites, setFavorites] = useState<Artist[]>([]);
  
  const HyperPopArtists = artistsData.filter((artist) => artist.genre === "HyperPop");
  ...
  return (
    <div>
      ...
      <main style={{ padding: 20 }}>
        <h1 style={{fontSize:'2rem'}}>Hyper Pop Artists</h1>
        
        {HyperPopArtists.length === 0 ? (
          <p>No HyperPop artists found.</p>
        ) : (
          <div style={{ 
            display: 'flex', 
            flexWrap: 'wrap', 
